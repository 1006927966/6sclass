...
from utils.eval import eval_fuse, displaymetric, eval_multi_fuse
import time
...
for epoch in range(EPOCHS):
    ...
    for i, data in enumerate(trainloader):
        ...
        if MIXUP:
            ...
            clssipre, binarypre = net(img)
            pre = [clssipre, binarypre]
            loss = 0
            for i in range(2):
                single_loss = torch.nn.CrossEntropyLoss()(pre[i], label[:,i])
                train_loss += single_loss * batch_size
                loss += single_loss
                total += batch_size
        loss.backward()
        optimizer.step()
    ...
    acc, recalldic, precisiondic = eval_multi_fuse(testloader, net, torch.cuda.device_count())
    if acc > best_acc:
        best_acc = acc
        best_epoch = epoch
