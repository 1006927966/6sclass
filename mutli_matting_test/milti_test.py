...
class ClassifierApi(object):
    ...
    def _construct_model(self, model_path):
        model = get_network('multilabel', 22)
    ...
    def _infer(self, x):
        with torch.no_grad():
            if self.n_gpu > 1:
                y1, y2 = torch.nn.parallel.data_parallel(self.model, x, self.gpu_list)
            else:
                y1, y2 = self.model(x)
...
def test(epoch, project, dirname):
    my_inference = ClassifierApi(model_path='/defaultShare/share/wujl/83/master_models/resnext101_22bg/model_{}.pth'.format(epoch), n_gpu=1)
    ...
    data_loader = DataLoader(data_set, batch_size=6, num_workers=4, shuffle=False)
    value1_list = list()
    label1_list = list()
    value2_list = list()
    label2_list = list()
