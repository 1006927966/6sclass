    ...
    def __init__(self,  transform=None):
        ...
        if transform is None:
            transform = torchvision.transforms.Compose([
                ...
                torchvision.transforms.RandomResizedCrop(224, scale=(0.8, 1), ratio=(0.8, 1.2), interpolation=2)
        ...
        maxnum = max([len(os.listdir(subpath)) for subpath in subpaths])
        for subpath in subpaths:
            names = os.listdir(subpath)
            randomnames = self.randomselect(names, maxnum)
            for randomname in randomnames:
                self.img_list.append(os.path.join(subpath, randomname))

    def randomselect(self, namelist, maxnum):
        getnum = len(namelist)
        usefullist = []
        for i in range(maxnum):
            index = random.randint(1, getnum)
            random.shuffle(namelist)
            usefullist.append(namelist[index-1])
        return usefullist
