...
if LOADING:
    ...
    for k, v in statedict.items():
        if 'fc' not in k:
            ...
            continue
    modeldic = net.state_dict()
    modeldic.update(newdic)
    net.load_state_dict(modeldic)
...
optimizer = torch.optim.SGD(filter(lambda p: p.requires_grad, net.parameters()), lr=INITLR, momentum=0.9, weight_decay=WD)
