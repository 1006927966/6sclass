...
if __name__ == '__main__':
    ...
    def test(epoch,  project, version):
        my_inference = ClassifierApi(model_path='/VisualGroup/share/wujl/83/master_models/bcnn_25/model_{}_frozen.pt'.format(epoch), n_gpu=1, device='cpu')
        img_dir = '/VisualGroup/share/wujl/83/test/{}/{}'.format(project, version)
        txt_dir = '/VisualGroup/share/wujl/83/test/{}/txtpath/'.format(project)
        os.makedirs(txt_dir, exist_ok=True)
        print(img_dir)
        save_txt = os.path.join(txt_dir, '{}_{}_bcnn.txt'.format(version, epoch))
        print(save_txt)
        ...
        for index, img in enumerate(data_loader):
           # print(img)
           # np.savez('/VisualGroup/share/wujl/83/test/9_21/croppic/tt', img.numpy())
    ...
    epochs = [60]
    project= 'testcls'
    days = ['bg_test', 'recall_test']
    for epoch in epochs:
        for day in days:
            print('[*]! the epoch {} begin'.format(epoch))
            test(epoch, project, day)
