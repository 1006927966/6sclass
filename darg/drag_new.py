...
def get_polygon_info(m_beg_date, beg_time, m_end_date, end_time):
    ...
    db_01 = mysql.connect(
        host = "10.88.0.48",
        user = "visionreadonly",
        passwd = "visionreadonly",
        database = "vision",
        auth_plugin="mysql_native_password",
    )

    print("Connect to Database")
    cursor_00 = db_00.cursor()
    cursor_01 = db_01.cursor()

    query = "SELECT capture_url, region, rectify, audit, event_type, confidence, time, time FROM inspect2 " \
            "where time>='%s %s' and time<='%s %s' ORDER BY time DESC"%(m_beg_date, beg_time, m_end_date, end_time)

    print(query)
    cursor_00.execute(query)
    cursor_01.execute(query)
    ## fetching all records from the 'cursor' object
    print("Fetch query data")
    records = cursor_01.fetchall()

    if len(records) == 0:
        records = cursor_00.fetchall()

    db_00.close()
    db_01.close()

    xml_list = list()
    boxes_list_count = list()
    repeat = []
    for record in records:
        if record[0] is None or record[4] not in [1, 2] :
            continue
        if 'jpg' in record[0]:
            print(record[0])
            break
        ...
        if poly_str is not None and len(poly_str) > 4:
            ...
            for poly in poly_list:
                ...
                if len(poly.split('type')[1]) > 10:
                    ddb_type = poly.split('type')[1].split('["')[1].split('"]')[0]
                else:
                    ddb_type = 'others'
                boxes_list.append({'xmin': min(point_x_list),
                                   ...
                                   'type': ddb_type})
                boxes_list_count.append(ddb_type)
        else:
            boxes_list = None
        img_name = os.path.split(record[0])[1]
# add the id for not repeat id
    ...
    return xml_list, boxes_list_count
...
if __name__ == '__main__':
    ...
    for dt in rrule(DAILY, dtstart=beg_date, until=end_date):
        ...
        boxes_info, boxlistcount = get_polygon_info(m_beg_date=date_time,
                                      beg_time='00:00:00',
