...
def calrecall(txtpath, prepath, maplist, savedir, imgdir, thresh, needlist, combine=True):
    ...
    for line in lines:
        ...
        picname = factors[0]
        odic[picname] = name
    with open(prepath, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    for line in lines:
        line = line.strip()
        factors = line.split(',')
...
def recallnum(needlist, iou, epoch, classiscore, maplist, project, detscore, rate):
    croprecalltxt = '/defaultShare/share/wujl/83/test/{}/txtpath/croprecall_{}_{}_{}.txt'.format(project, detscore, iou, rate)
    prerecalltxt =  '/defaultShare/share/wujl/83/test/{}/txtpath/croprecall_{}_{}_{}_{}.txt'.format(project, detscore, iou, rate, epoch)
...
def alertnum(maplist, classiscore, project, epoch, detscore, rate):
    pretxtpath = '/defaultShare/share/wujl/83/test/{}/txtpath/croppre_{}_{}_{}.txt'.format(project, detscore, rate, epoch)
...
if __name__ == '__main__':
    ...
    ious = [0.01]
    needlist = ['xianshu_luan', 'luanfang',  'laji00', 'laji01', 'laji03', 'laji02', 'pip_l', 'bancai_luan', 'hanzha_l',
                'zhixiang_luan', 'xiaojian_luan', 'zangwu', 'zhufei']
    rate = 0.1
    epochs = [20]
    project = '9_21'
    classiscores = [0]
    ...
    for iou in ious:
        ...
        for epoch in epochs:
            for classiscore in classiscores:
                ...
                total = recallnum(needlist, iou, epoch, classiscore, maplist, project, detscore, rate)
                if total>maxtotal:
                    maxtotal = total
                    maxepoch = epoch
    ...
    for key in maxdic.keys():
        ...
        for classiscore in classiscores:
            ...
            alertnum(maplist, classiscore, project, maxepoch, detscore, rate)
